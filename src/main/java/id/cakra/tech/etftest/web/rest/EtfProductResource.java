...
/**
 * REST controller for managing {@link id.cakra.tech.etftest.domain.EtfProduct}.
 */
@RestController
@RequestMapping("/api")
public class EtfProductResource {
    ...
    public ResponseEntity<EtfProductDTO> updateEtfProduct(@Valid @RequestBody EtfProductDTO etfProductDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, etfProductDTO.getId().toString()))
            .body(result);
    }
    ...
    public List<EtfProductDTO> getAllEtfProducts(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
